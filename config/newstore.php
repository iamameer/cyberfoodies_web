<?php
    ...
    if(isset($_COOKIE['q'])){
        ...
        $store_id = $user_id . '_'. substr($store_name, 0, 5);;
    }else{
        $user_id = "uid";
        $store_id = "sid";
    }
    ...
    if(!$conn-> query($sql)){
        ...
        echo "<script type='text/javascript'>alert('Successfully added Store!')
        window.location.replace('http://cyberfoodies.epizy.com/profile.php'); </script>";
        // header("profile.php");
        // exit();
    }
